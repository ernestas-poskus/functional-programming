...
(define (racketlist->mupllist lst)
  (cond ((equal? lst '()) (aunit))
        ((not (pair? lst)) lst)
        (else (apair (racketlist->mupllist (car lst)) (racketlist->mupllist (cdr lst))))))


(define (mupllist->racketlist lst)
  (cond ((aunit? lst) '())
        ((not (apair? lst)) lst)
        (else (cons (mupllist->racketlist (apair-e1 lst)) (mupllist->racketlist (apair-e2 lst))))))
...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(call? e) (let* ([clos (eval-under-env (call-funexp e) env)]
                                  ...
                                  [bod (fun-body func)])
                              (if (fun-nameopt func)
                                  [eval-under-env bod (cons (cons (fun-nameopt func) clos) final-env)]
                                  [eval-under-env bod final-env]))]
